...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            with trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar) as itereation_tqdm: 
                for _ in itereation_tqdm:
                    ...
                    if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                        _ = self._eval_during_training(evaluator, output_path, save_best_model, save_ckpt_on_eval, epoch, training_steps, callback, name="Eval")
            ...
            print(f"Train Loss = {loss_value.item():>8.4f}   Accuracy = {accuracy:>6.4f}   AUC = {AUC:>6.4f}" )
            ...
            if tester is not None:
                _ = self._eval_during_training(tester, output_path, save_best_model, save_ckpt_on_eval, epoch, training_steps, callback, name="Test")
            if zero_shot_tester is not None:
                _ = self._eval_during_training(zero_shot_tester, output_path, save_best_model, save_ckpt_on_eval, epoch, training_steps, callback, name="Zero")
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, save_ckpt_on_eval, epoch, steps, callback, name):
