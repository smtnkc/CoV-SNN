...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description="Train CoV-RoBERTa Classifier")
...
OUTPUT_DIR = f"../outputs/variant_classification/covroberta"
DATA_DIR = "../data"
BATCH_SIZE = 4 
...
def main():
    ...
    logger.info(f"Configuration: MAX_LENGTH={MAX_LENGTH}, MODEL_PATH={MODEL_PATH}")
    ...
    logger.info(f"Loading tokenizer from 'trained_tokenizer/'...")
    try:
        tokenizer = RobertaTokenizerFast.from_pretrained("trained_tokenizer/")
    except Exception as e:
        logger.warning(f"Could not load tokenizer from 'trained_tokenizer/', trying {MODEL_PATH}: {e}")
        try:
            tokenizer = RobertaTokenizerFast.from_pretrained(MODEL_PATH)
        except Exception as e2:
             logger.error(f"Could not load tokenizer from {MODEL_PATH} either: {e2}")
             raise e2

    def tokenize_function(examples):
        sequences = examples["sequence"]
        return tokenizer(sequences, padding=False, truncation=True, max_length=MAX_LENGTH)
