...
# --- Configuration ---
MODEL_PATH = "../../model_ESM2_coronaviridae"
BASE_MODEL_NAME = "facebook/esm2_t33_650M_UR50D" # For tokenizer and config fallback
DATA_DIR = "../data"
OUTPUT_DIR = "../outputs/variant_classification/esm2"
MAX_LENGTH = 1024 # ESM2 max length
BATCH_SIZE = 4 # Adjust based on GPU memory (650M param model is large)
    ...
    for variant, filename in FILES.items():
        ...
        if os.path.exists(filepath):
            ...
            if 'sequence' not in df.columns:
                 # Fallback for variation in column names if any
                 # Based on file inspection, it is 'sequence'
...
def main():
    ...
    # We use the base ESM2 tokenizer. Domain adaptation usually preserves the vocabulary.
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label'])):
        ...
        try:
            model = AutoModelForSequenceClassification.from_pretrained(
                ...
                id2label=ID2LABEL,
                label2id=LABEL2ID,
                # If the config in the folder doesn't match standard ESM2, we might need to be careful,
                # but usually AutoModel handles it if config.json is present.
            )
        except Exception as e:
            logger.error(f"Error loading local model: {e}")
            logger.info(f"Fallback: Loading base model {BASE_MODEL_NAME} (weights will be reset for classification head)")
            model = AutoModelForSequenceClassification.from_pretrained(
                BASE_MODEL_NAME,
                num_labels=len(LABEL2ID),
                id2label=ID2LABEL,
